...
def _inepro_energy_values(values: dict) -> dict[int, float]:
    return {
        0x6000:_value(values,"e_total"), 0x6002:0.0, 0x6004:0.0, 0x6006:0.0, 0x6008:0.0, 0x600A:0.0,
        0x600C:_value(values,"e_import"), 0x600E:0.0, 0x6010:0.0, 0x6012:0.0, 0x6014:0.0, 0x6016:0.0,
        0x6018:_value(values,"e_export"), 0x601A:0.0, 0x601C:0.0, 0x601E:0.0, 0x6020:0.0, 0x6022:0.0,
        0x6024:0.0, 0x6026:0.0, 0x6028:0.0, 0x602A:0.0, 0x602C:0.0, 0x602E:0.0,
        0x6030:0.0, 0x6032:0.0, 0x6034:0.0, 0x6036:0.0, 0x6038:0.0, 0x603A:0.0,
        0x603C:0.0, 0x603E:0.0, 0x6040:0.0, 0x6042:0.0, 0x6044:0.0, 0x6046:0.0,
    }
...
def build_inepro_pro380(values: dict, word_order: str) -> Dict[int, int]:
    ...
    regs: Dict[int, int] = {}
    fields = {0x5000:"voltage_avg",0x5002:"u1",0x5004:"u2",0x5006:"u3",0x5008:"freq",0x500A:"current_total",0x500C:"i1",0x500E:"i2",0x5010:"i3",0x5012:"p_total",0x5014:"p1",0x5016:"p2",0x5018:"p3",0x501A:"q_total",0x501C:"q1",0x501E:"q2",0x5020:"q3",0x5022:"s_total",0x5024:"s1",0x5026:"s2",0x5028:"s3",0x502A:"pf_total",0x502C:"pf1",0x502E:"pf2",0x5030:"pf3"}
    ...
    regs[0x6048] = 0; _put_float(regs,enc,0x6049,0.0)
    return regs


def build_inepro_pro2(values: dict, word_order: str) -> Dict[int, int]:
    """Inepro PRO2-Mod single-phase map; PRO2 uses FLOAT32 ABCD."""
    ...
    # L2/L3 registers exist in the generic addressing scheme but are not physical PRO2 channels.
    ...
    for addr,value in _inepro_energy_values(values).items(): _put_float(regs,enc,addr,value)
    regs[0x6048] = 1; _put_float(regs,enc,0x6049,0.0)
    return regs


def _build_janitza_b_series(values: dict, single_phase: bool) -> Dict[int,int]:
    """Janitza B21/B23 Modbus 0x5Bxx map (UINT32/INT32, 0.1 V/0.01 A/W/Hz)."""
    regs: Dict[int,int] = {}
