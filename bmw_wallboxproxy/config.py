...
ENTITY_FIELDS = (
    ...
    ("power_offset", "Power offset", "Home Assistant entity for power offset in watts."),
)
ENTITY_DEFAULTS = {key: "" for key, _label, _help in ENTITY_FIELDS}
...
def _load_entity_settings() -> dict[str, str]:
    return {key: _env_str(_entity_env_name(key), default) for key, default in ENTITY_DEFAULTS.items()}
...
WEBAPP_SSL_MODE = _env_choice("WEBAPP_SSL_MODE", "off", {"off", "adhoc", "files"})
...
def get_ha_verify_tls() -> bool:
    return False if HA_USE_SUPERVISOR and SUPERVISOR_TOKEN else HA_VERIFY_TLS


def get_ha_auth_token() -> str:
    return SUPERVISOR_TOKEN if HA_USE_SUPERVISOR and SUPERVISOR_TOKEN else HA_TOKEN


def get_ha_auth_mode() -> str:
    if HA_USE_SUPERVISOR and SUPERVISOR_TOKEN:
        return "supervisor"
    return "manual_token" if HA_TOKEN else "none"
...
def get_ha_entity_fields() -> list[dict[str, str]]:
    return [{"key": key, "label": label, "help": help_text, "value": ENTITIES[key]} for key, label, help_text in ENTITY_FIELDS]
...
def update_ha_live_data_settings(ha_url: str, verify_tls: bool, entities: dict[str, str]) -> bool:
    ...
    normalized_url = HA_URL if is_supervisor_ha_mode() else ha_url.strip().rstrip("/")
    if not is_supervisor_ha_mode() and (not normalized_url or not _is_valid_url(normalized_url)):
    ...
    normalized_entities = {key: str(entities[key]).strip() for key, _label, _help in ENTITY_FIELDS}
    changed = HA_URL != normalized_url or HA_VERIFY_TLS != verify_tls or any(ENTITIES.get(k) != v for k, v in normalized_entities.items())
    HA_URL, HA_VERIFY_TLS = normalized_url, False if is_supervisor_ha_mode() else verify_tls
    ENTITIES.clear(); ENTITIES.update(normalized_entities)
    env_updates = {"HA_URL": HA_URL, "HA_VERIFY_TLS": "true" if HA_VERIFY_TLS else "false"}
    env_updates.update({_entity_env_name(key): value for key, value in normalized_entities.items()})
...
DEFAULTS = {
    ...
    "e_import_total": 0.0, "e_export_total": 0.0, "power_offset": 0.0,
}
