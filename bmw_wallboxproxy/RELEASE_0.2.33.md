...
The existing `inepro_500c_encoding` selector remains available with:

- `int32_ma_cdab`
- `float32_cdab`
- `float32_abcd`

The selected current encoding applies to `0x500C`, `0x500E` and `0x5010`. The response size remains determined by the Wallbox request: a two-register request returns 4 payload bytes and a 9-byte RTU frame including CRC.
