...
def _pro2_supported_word_addresses():
    ...
    pairs = {
        0x4000, 0x4005, 0x4007, 0x4009, 0x400D,
        0x5000, 0x5002, 0x5004, 0x5006, 0x5008, 0x500A,
        0x500C, 0x500E, 0x5010, 0x5012, 0x501A, 0x5022, 0x502A,
        0x6000, 0x6002, 0x6004, 0x600C, 0x600E, 0x6010, 0x6018,
        0x601A, 0x601C, 0x6024, 0x6026, 0x6028, 0x6030, 0x6032,
        0x6034, 0x603C, 0x603E, 0x6040, 0x6049,
    }
    words = set(single)
    for addr in pairs:
...
def _pro380_only_word_addresses():
    pairs = {
        0x5014, 0x5016, 0x5018, 0x501C, 0x501E, 0x5020, 0x5024,
        0x5026, 0x5028, 0x502C, 0x502E, 0x5030, 0x6006, 0x6008,
        0x600A, 0x6012, 0x6014, 0x6016, 0x601E, 0x6020, 0x6022,
        0x602A, 0x602C, 0x602E, 0x6036, 0x6038, 0x603A, 0x6042,
        0x6044, 0x6046,
    }
    words = set()
    for addr in pairs:
...
def test_pro2_float_encoding_respects_word_order():
    values = {
        "voltage_avg": 230.0,
        "u1": 230.0,
        "freq": 50.0,
    ...
    assert (abcd[0x500C], abcd[0x500D]) == (0x4180, 0x0000)
    assert (cdab[0x500C], cdab[0x500D]) == (0x0000, 0x4180)
    assert math_is_close(_f32(abcd, 0x500C), 16.0)
