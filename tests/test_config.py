...
def test_meter_model_is_loaded_from_environment(monkeypatch):
    monkeypatch.setenv("METER_MODEL","inepro_pro2"); reloaded=importlib.reload(config); assert reloaded.get_meter_model()=="inepro_pro2"


def test_meter_model_supports_all_profiles(monkeypatch):
    for model in ("inepro_pro380","inepro_pro2","janitza_b23","janitza_b21"):
        monkeypatch.setenv("METER_MODEL",model); reloaded=importlib.reload(config); assert reloaded.get_meter_model()==model


def test_invalid_meter_model_falls_back_to_default(monkeypatch):
    monkeypatch.setenv("METER_MODEL","unsupported"); reloaded=importlib.reload(config); assert reloaded.get_meter_model()=="inepro_pro380"
